...
const NOMBRE_MODULO = { empresa: "Inversión empresa", servicios: "Inversión servicios", activo: "Inversión activo" };
...
export default function PanelAdmin({ abrirProyecto }) {
  ...
  return (
    <>
      <Card title="Padrón"
        sub={`${alumnos.length} ${alumnos.length === 1 ? "cuenta" : "cuentas"} de alumno · ${alumnos.filter((x) => x.activo !== false).length} activas · ${new Set(filas.map((f) => f.usuario)).size} con trabajo guardado`}
