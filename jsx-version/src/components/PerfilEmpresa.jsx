...
function NumOpt({ value, onChange, tipo, placeholder }) {
...
function Pregunta({ etiqueta, children }) {
  return (
    <div>
...
export default function PerfilEmpresa({ s, up }) {
  const e = s.empresa || {};
  const d = e.diagnostico || {};
  const st = estadoPerfil(s);
  const avance = Math.min(1, st.palabras / MIN_PALABRAS);
  const setD = (k, v) => up((n) => { n.empresa.diagnostico = { ...(n.empresa.diagnostico || {}), [k]: v }; });

  const entrada = (p) => {
    const v = d[p.k];
    if (p.tipo === "txt") return <TxtIn value={v} placeholder={p.ph} onChange={(x) => setD(p.k, x)} />;
    ...
    return <NumOpt value={v} tipo={p.tipo} placeholder={p.ph} onChange={(x) => setD(p.k, x)} />;
  };
    ...
    <>
      <Card title="Descripción del negocio"
        ...
        <textarea className={inputCls} rows={8} value={e.descripcion || ""} placeholder={GUIA}
          ...
          onChange={(ev) => up((n) => { n.empresa.descripcion = ev.target.value; })} />
          ...
          {SECCIONES.map((sec) => (
            <div key={sec.titulo} className="rounded-lg p-3" style={{ background: C.soft, border: `1px solid ${C.line}` }}>
              ...
              <div className="flex flex-col gap-2.5">
                {sec.preguntas.map((p) => <Pregunta key={p.k} etiqueta={p.etiqueta}>{entrada(p)}</Pregunta>)}
              </div>
            </div>
          ))}
        </div>
