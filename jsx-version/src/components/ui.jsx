...
export function NumIn({ value, onChange, dec = 2, suffix, align = "right", disabled, plain, moneda }) {
...
export function TxtArea({ value, onChange, placeholder, rows = 4 }) {
  return <textarea className={inputCls} style={{ ...inputSt, resize: "vertical", lineHeight: 1.55 }} rows={rows}
    value={value || ""} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} />;
}

export function Slider({ value, onChange, min = 0, max = 100, step = 0.5, tramos, color }) {
...
export function UnidadIn({ value, onChange }) {
...
export function AvisoMezcla({ productos, L }) {
...
export function ChipMezcla({ productos }) {
  const lista = productos || [];
  if (!lista.length) return null;
  const total = lista.reduce((a, p) => a + (p.mix || 0), 0);
  const dif = 1 - total;
