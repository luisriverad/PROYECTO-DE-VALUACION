...
export function tieneDatos(t) {
  if (!t) return false;
  const cifras = Object.values(t.cifras || {}).some((v) => typeof v === "number" && isFinite(v));
  return !!(t.resumen || (t.hallazgos || []).length || (t.notas || "").trim() || cifras);
}

/* ---------- a qué pestaña mandar cada fricción ---------- */
export function pestanas(L) {
  return {
...
export function leerJSON(res) {
  let txt = res?.texto || "";
  const bloques = res?.data?.content;
  if (Array.isArray(bloques)) {
    let ult = -1;
    bloques.forEach((b, i) => { if (b?.type && b.type !== "text") ult = i; });
    const t = bloques.slice(ult + 1).filter((b) => b?.type === "text").map((b) => b.text).join("");
    if (t.includes("{")) txt = t;
  }
...
export function fuentesDe(res) {
  const out = [];
  const vistos = new Set();
  const poner = (url, titulo) => {
...
function zonaDe(s) {
  ...
  return [d.ciudad, d.estado, d.pais].map((x) => String(x || "").trim()).filter(Boolean).join(", ");
}
...
function queEsElNegocio(s, L) {
  const prods = (s.productos || []).map((p) => p.nombre).filter(Boolean);
  return [
...
export function hayDescripcion(s) {
  return perfilListo(s);
}

/* ---------- investigar un frente ---------- */
export async function investigarTema(k, s, L) {
  const T = TEMAS.find((t) => t.k === k);
  if (!T) throw new Error("Frente de investigación desconocido.");
  const llaves = T.cifras.map((c) => `"${c.k}": número o null`).join(", ");
  const explica = T.cifras.map((c) => `- ${c.k}: ${c.etiqueta}${c.ayuda ? " (" + c.ayuda + ")" : ""}`).join("\n");
  const zona = zonaDe(s);
...
Entre 6 y 12 hallazgos.${T.cifras.length ? `\nEn "cifras" usa exactamente estas llaves:\n${explica}` : `\n"cifras" va vacío: {}`}`,
  ...
  return {
    ...
    hallazgos: (Array.isArray(j?.hallazgos) ? j.hallazgos : []).slice(0, 12).map((h) => ({
...
export function contrasteNumerico(s, m) {
  const T = (s.investigacion || {}).temas || {};
  const cif = (tema, k) => { const v = T[tema]?.cifras?.[k]; return typeof v === "number" && isFinite(v) ? v : null; };
  const filas = [];
  const pendientes = [];
  const fila = (o) => filas.push(o);
  const falta = (supuesto, tema) => pendientes.push({ supuesto, tema });
  const pp = (v) => (v >= 0 ? "+" : "−") + num(Math.abs(v) * 100, 1) + " pp";
  ...
  const crecs = ((s.plan || {}).crec || []).slice(0, Math.max(H - 1, 1)).filter((x) => typeof x === "number");
  const crecP = crecs.length ? crecs.reduce((a, b) => a + b, 0) / crecs.length : null;
  ...
  const gente = (s.recursosMO || []).filter((r) => r.sueldoMensual > 0);
  if (sueldoM == null) falta("Sueldos del personal", "costos");
  else if (gente.length && sueldoM > 0) {
    const personas = gente.reduce((a, r) => a + (r.personas || 1), 0);
    const prom = gente.reduce((a, r) => a + r.sueldoMensual * (r.personas || 1), 0) / personas;
  ...
  /* fiscal */
  for (const [k, nombre] of [["isr", "Tasa de ISR"], ["ptu", "PTU"]]) {
...
function fotoDelModelo(s, m) {
  const r = (v) => (typeof v === "number" && isFinite(v) ? Math.round(v) : null);
  const d = (v, k = 4) => (typeof v === "number" && isFinite(v) ? +v.toFixed(k) : null);
  const a1 = m?.anios?.[0] || {};
  return {
    ...
    productos: (m?.prod || []).map((p) => ({ nombre: p.nombre, precio: p.precio, mezcla: p.mix, costoEstandar: r(p.estandar), margenReal: d(p.margenReal) })),
    planDeVentas: { unidadesMensualesAnio1: s.plan?.unidadesMes, crecimientoAnual: s.plan?.crec, unidadesPorAnio: (m?.unidadesAnio || []).map(r) },
    ventasPorAnio: (m?.anios || []).map((x) => r(x.ventas)),
    ebitdaPorAnio: (m?.anios || []).map((x) => r(x.ebitda)),
    ...
    personal: (s.recursosMO || []).map((x) => ({ puesto: x.nombre, sueldoMensual: x.sueldoMensual, personas: x.personas })),
    insumos: (s.insumos || []).map((x) => ({ nombre: x.nombre, costoUnitario: x.volumenLote ? d(x.costoLote / x.volumenLote, 2) : null, unidad: x.unidad })),
    inversiones: (s.activos || []).map((x) => ({ nombre: x.nombre, monto: x.inversion })),
...
function investigacionParaIA(s) {
  const T = (s.investigacion || {}).temas || {};
  return TEMAS.filter((t) => tieneDatos(T[t.k])).map((t) => {
    ...
    return {
      ...
      hallazgos: (x.hallazgos || []).map((h) => ({ dato: h.dato, valor: h.valor, fuente: h.fuente, fecha: h.fecha })),
      implicaciones: x.implicaciones, notasDelEmpresario: x.notas || undefined,
    };
  });
}

export async function correrContraste(s, m, L, numerico) {
  ...
  const yaVistas = numerico.filas.filter((f) => f.sev !== "ok").map((f) => `${f.supuesto}: modelo ${f.modelo} vs. mercado ${f.mercado} (${f.sev})`);
...
{"veredicto":"2 a 3 líneas","consistencia":0,"fricciones":[{"area":"...","supuesto":"...","valorModelo":"...","evidencia":"...","valorMercado":"...","severidad":"alta, media o baja","impacto":"qué le pasa al VPN, la TIR o la caja si la realidad gana","recomendacion":"qué cambiar en el modelo, concreto","tab":"..."}],"puntosCiegos":["..."],"fortalezas":["..."]}`,
  ...
  const j = leerJSON(res);
  const sevOk = (v) => (["alta", "media", "baja"].includes(String(v).toLowerCase()) ? String(v).toLowerCase() : "media");
  ...
  return {
    ...
    fricciones: (Array.isArray(j?.fricciones) ? j.fricciones : []).slice(0, 12).map((f) => ({
    ...
    })).sort((a, b) => SEV[a.severidad].orden - SEV[b.severidad].orden),
