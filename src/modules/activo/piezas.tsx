...
export const SecHead = ({ children }: any) => (
...
export const Row = ({ label, hint, children, ancho = 130 }: any) => (
...
export function Campo({ A, up, g, k, label, hint, tipo = "money", dec }: any) {
...
export function FlowTable({ Y, rows }: any) {
  return (
...
/* ---------- tabla genérica ---------- */
export function GridTable({ head, rows }: any) {
  return (
    <div style={{ overflowX: "auto" }}>
      <table className="w-full" style={{ borderCollapse: "collapse" }}>
        <thead>
...
export function AreaChart({ vals, label, unidad = "Año" }: any) {
  const n = vals.length;
...
export function BarsChart({ items }: any) {
...
export const Head = ({ titulo, texto }: any) => (
  <div className="mb-4">
