...
function ValorIn({ value, onChange }: any) {
...
export function Severidad({ sev }: any) {
...
function CifraIn({ c, value, onChange }: any) {
...
function BotonContraste({ onClick, disabled, children = "CONTRASTE" }: any) {
  return (
...
/* ============================================================
   ...
   ============================================================ */
export function TabInvestigacion({ s, up, L, irA, flash }: any) {
  ...
  const setTema = (k: string, fn: any) => up((n: any) => {
    const I = asegurar(n);
    if (!I.temas[k]) I.temas[k] = temaVacio();
    fn(I.temas[k]);
  });

  const investigar = async (k: string) => {
    setEstado((e: any) => ({ ...e, [k]: "cargando" }));
    try {
      const r = await investigarTema(k, s, L);
      /* las notas son del empresario: volver a investigar no las pisa */
      up((n: any) => { const I = asegurar(n); I.temas[k] = { ...r, notas: I.temas[k]?.notas || "" }; });
      setEstado((e: any) => ({ ...e, [k]: null }));
    } catch (e: any) {
      setEstado((x: any) => ({ ...x, [k]: { error: e?.message || "No se pudo investigar." } }));
    }
  };
    ...
    <>
      ...
      <Card title={T.titulo} sub={T.pregunta}
              ...
              {T.cifras.map((c: any) => (
                <Field key={c.k} label={c.etiqueta} hint={c.ayuda}>
                  <CifraIn c={c} value={d.cifras?.[c.k]}
                    onChange={(v: any) => setTema(activo, (t: any) => { t.cifras = { ...(t.cifras || {}), [c.k]: v }; })} />
                </Field>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          <Btn small onClick={() => setTema(activo, (t: any) => {
        ...
        {(d.hallazgos || []).length ? (
          <div className="overflow-x-auto mb-4">
            <table className="w-full" style={{ borderCollapse: "collapse" }}>
              ...
              <tbody>
                {d.hallazgos.map((h: any, i: number) => {
                  const campo = (k: string) => (v: any) => setTema(activo, (t: any) => { t.hallazgos[i] = { ...t.hallazgos[i], [k]: v }; });
                  return (
                    <tr key={h.id || i}>
                      ...
                      <td className="px-1 py-1 text-center">
                        <button title="Quitar este hallazgo" className="text-[13px] hover:opacity-60" style={{ color: C.muted }}
                          onClick={() => setTema(activo, (t: any) => { t.hallazgos.splice(i, 1); })}>✕</button>
                      </td>
                    </tr>
                  );
                })}
        ...
        <Field label="Tus notas de campo"
          ...
          <TxtArea rows={3} value={d.notas} onChange={(v: any) => setTema(activo, (t: any) => { t.notas = v; })}
...
/* ============================================================
   PESTAÑA: CONTRASTE
   ============================================================ */
export function TabContraste({ s, up, m, L, irA }: any) {
  const inv = s.investigacion || {};
  const temas = inv.temas || {};
  ...
  const cubiertos = TEMAS.filter((t) => tieneDatos(temas[t.k])).length;
  const friccNum = numerico.filas.filter((f: any) => f.sev !== "ok");
  const altasNum = friccNum.filter((f: any) => f.sev === "alta").length;
  ...
  const correr = async () => {
    ...
    try {
      ...
      up((n: any) => { asegurar(n).contraste = r; });
    } catch (e: any) {
  ...
  const IrA = ({ tab }: any) => tab && tabs[tab] ? (
    <button onClick={() => irA?.(tab)} className="text-[11px] font-medium hover:underline whitespace-nowrap" style={{ color: C.azul }}>
      {tabs[tab]} →
    </button>
  ) : null;

  const nombreTema = (k: string) => TEMAS.find((t) => t.k === k)?.titulo || k;
  const pendientesPorTema: any = {};
      ...
      <Card title="Contraste · lo supuesto vs. lo que está pasando allá afuera"
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))" }}>
          ...
          <KPI label="Fricciones de fondo (IA)" value={res ? String(res.fricciones.length) : "—"}
            sub={res ? `${res.fricciones.filter((f: any) => f.severidad === "alta").length} de severidad alta` : "corre CONTRASTE"} />
        ...
        {numerico.filas.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full" style={{ borderCollapse: "collapse" }}>
              ...
              <tbody>
                {numerico.filas.map((f: any, i: number) => (
          ...
          <>
            ...
            {res.fricciones.length === 0 ? (
              ...
              <div className="flex flex-col gap-2 mb-4">
                {res.fricciones.map((f: any, i: number) => (
            ...
            <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))" }}>
              <div>
                ...
                {res.puntosCiegos.length ? (
                  <ul className="text-[12px] leading-relaxed list-disc pl-4">{res.puntosCiegos.map((p: any, i: number) => <li key={i}>{p}</li>)}</ul>
              ...
              <div>
                ...
                {res.fortalezas.length ? (
                  <ul className="text-[12px] leading-relaxed list-disc pl-4">{res.fortalezas.map((p: any, i: number) => <li key={i}>{p}</li>)}</ul>
