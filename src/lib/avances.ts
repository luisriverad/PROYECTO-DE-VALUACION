...
export function resumenEmpresa(estado: any, m: any) {
  return {
...
export function resumenActivo(estado: any) {
  const A = estado || {};
...
export async function cargarAvances(usuario: string) {
...
let avisar: ((e: Estado, detalle?: string) => void) | null = null;
export function alGuardar(fn: any) { avisar = fn; }

export function guardarAvance(usuario: string, modulo: string, estado: any, resumen: any) {
  if (!configurado || !usuario) return;
...
export async function guardarTodo(usuario: string, piezas: any[]) {
...
async function escribir(usuario: string, modulo: string, estado: any, resumen: any) {
  try {
...
/* Trae el trabajo completo de una persona. Sólo devuelve algo si quien
   ...
   no este código. */
export async function traerAvancesDe(usuario: string) {
  if (!configurado || !usuario) return {};
  const { data, error } = await sb.from("avances").select("modulo, estado").eq("usuario", usuario);
